...
void TestRobustSuspension::testActive_1() {
	...
	shared_ptr<vector<double>> result_x = robust_suspension.active(ms, w);

	for(unsigned int i = 0; i < result_x->size(); i++)
		checkEqual(result_x->at(i), ref_x[i], 1e-4, " ");
}
...
void TestRobustSuspension::testActive_2() {
	...
	for(unsigned int i = 0; i < result_x->size(); i++)
		checkEqual(result_x->at(i), ref_x_1[i], 1e-4, " ");
	...
	for(unsigned int i = 0; i < result_x->size(); i++)
		checkEqual(result_x->at(i), ref_x_2[i], 1e-4, " ");
